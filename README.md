...
This repository contains the **training recipe, identity-data tooling, and ops scripts**. The **weights** are on [Hugging Face](https://huggingface.co/williamyates/WillyGPT). The **full build report** — eval deep-dive and mechanistic analysis, rendered — is at [willygpt.netlify.app](https://willygpt.netlify.app). Base pretraining clears the GPT-2 CORE reference (0.3016 vs 0.2565); the model ships as two checkpoints (SFT and RL) whose trade-offs are characterized below.
...
The model quits before it answers. Suppressing `<|assistant_end|>` for the first ~80 decode steps (a `min_new_tokens` constraint, inference only, no weight changes) recovers the identical correct trace and restores SpellingBee from ~18% to **~95%**. A constant end-of-turn logit penalty (`WILLY_END_PENALTY≈10`) is an equivalent, schedule-free alternative.
...
A deeper causal account (direct logit attribution → activation patching → logit-lens) traces the regression to a content-state feature that switches on at layer 9, is carried up the residual stream, and is read out into the end-of-turn logit at the final MLP — recoverable at the output logit but not by dense residual edits. Full writeup at [willygpt.netlify.app](https://willygpt.netlify.app).
...
- **Min-length decoding for the RL checkpoint.** Suppressing `<|assistant_end|>` for the first ~80 generated tokens (or a constant `WILLY_END_PENALTY≈10` end-logit penalty) restores SpellingBee from ~18% to ~95% greedy.
